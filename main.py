import os
# Авто-установка библиотек при первом запуске на хостинге
os.system("pip install pyTelegramBotAPI curl_cffi")
...
# --- КОНФИГУРАЦИЯ ---
# Бот берет токен из переменных окружения сервера (безопасно)
...
def extract_clean_schedule(data, day_num):
    """Очистка данных от системного мусора сайта для точного сравнения"""
...
def format_day(data, target_date):
    if not data or 'lessons' not in data: return "❌ Данные не найдены."
...
def set_g(c):
    ...
    bot.edit_message_text("✅ Расписание привязано!", c.message.chat.id, c.message.message_id)
    bot.send_message(c.message.chat.id, "Используй кнопки меню:", reply_markup=main_menu())
